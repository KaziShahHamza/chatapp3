...
import { fetchRequest, voteRequest, commentRequest, voteRequestComment, toggleResolved } from '../services/requestApi';
...
export default function RequestDetails() {
  ...
  const load = async () => { setRequest(await fetchRequest(id)); };
  useEffect(() => { load(); }, [id]);
  if (!request) return <p className="p-4">Loading...</p>;

  const isAuthor = token && request.author._id === JSON.parse(atob(token.split('.')[1])).id;
    ...
    <div className="max-w-3xl mx-auto p-4 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-gray-500">
          ...
          <Link to={`/profile/${request.author._id}`} className="hover:underline">
            {request.author.name}
          </Link> • {timeAgo(request.createdAt)}
        </p>
      </div>

      {/* Body */}
      <div className="bg-white border rounded p-4 shadow-sm text-gray-800">
        {request.body}
      </div>
        ...
        {request.comments.map(c => (
          <div key={c._id} className="space-y-1 border rounded p-3 bg-gray-50">
            ...
            <VoteButtons
              ...
              myVote={c.myVote}
              disabled={!token}
              onLike={() => voteRequestComment(id, c._id, 1, token).then(load)}
              onDislike={() => voteRequestComment(id, c._id, -1, token).then(load)}
            />
          </div>
        ))}
      </div>
